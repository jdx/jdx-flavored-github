...
test('loads another inbox page only while the active view stays short', () => {
  ...
  assert.equal(shouldLoadExtraNotificationPage({...baseState, enabled: false}), false);
  assert.equal(shouldLoadExtraNotificationPage({...baseState, hasNextPage: false}), false);
  assert.equal(shouldLoadExtraNotificationPage({...baseState, visibleCount: 25}), false);
  assert.equal(shouldLoadExtraNotificationPage({...baseState, loadedPages: 5}), false);
});

test('stops trusting a Next link that was hidden for an exhausted inbox', () => {
  const window = inbox('<a aria-label="Next" href="/notifications?page=4">Next</a>');
  const link = window.document.querySelector('a[aria-label="Next"]') as unknown as Element;
...
test('merges a fetched page into the repository groups already on screen', () => {
  ...
  const fetched = new window.DOMParser().parseFromString(group('jdx/mise', [1, 2]), 'text/html');
  ...
  assert.deepEqual(
    [...document.querySelectorAll('.js-notifications-list > .notifications-list-item')].map(
      (row) => (row as unknown as HTMLElement).dataset.notificationId,
    ),
    ['1', '2'],
  );
});
...
test('appends a repository group the loaded pages have not shown yet', () => {
  ...
  assert.deepEqual(
    groups.map((element) =>
      [...element.querySelectorAll('.notifications-list-item')].map(
        (row) => (row as unknown as HTMLElement).dataset.notificationId,
      ),
    ),
    [
