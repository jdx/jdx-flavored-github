import type {NotificationRule, Surface, ViewDefinition} from '../shared/types.js';
...
const builtInViews: Record<Surface, ViewDefinition[]> = {
  ...
  pulls: [
    {id: 'ready', label: 'Ready', dsl: 'is:open is:pr draft:false'},
    {
      ...
      dsl: 'is:open is:pr draft:false user-review-requested:@me',
    },
...
function validateBulkActions(items: ViewDefinition[], surface: Surface) {
  ...
  for (const item of items) {
    ...
    for (const action of item.actions ?? []) {
      if (!/^[a-z0-9][a-z0-9-]*$/.test(action.id) || actionIds.has(action.id)) {
      ...
      const nativeStepIndex = action.steps.findIndex(
        (step) =>
          step.type.startsWith('issue:') ||
          step.type.startsWith('pr:') ||
          step.type.startsWith('label:'),
      );
      if (nativeStepIndex >= 0 && nativeStepIndex !== action.steps.length - 1) {
        throw new Error(
...
function tokenize(source) {
  ...
  let index = 0;
  const tokenPattern =
    /\s*(\(|\)|and\b|or\b|not\b|-?title:\/(?:\\.|[^/])*\/[dgimsuvy]*|-?label:"(?:\\.|[^"])*"|-?[^\s()]+)/iy;
...
function parseNotificationDsl(source) {
  ...
  function validateAtom(token) {
    ...
    if (
      ...
      (labelMatch && decodeQuotedValue(labelMatch[1] ?? labelMatch[2]).length > 0)
    ) {
      return;
    }
    ...
    if (titleMatch) {
      // Constructed only to surface a SyntaxError for an invalid pattern.
...
function evaluateNotificationDsl(tree, facts, resolveRule) {
  switch (tree.type) {
    case 'and': {
      return (
        evaluateNotificationDsl(tree.left, facts, resolveRule) &&
...
function evaluateAtom(rawAtom, facts, resolveRule) {
  ...
  if (qualifier === 'label') {
    ...
    const label = decodeQuotedValue(labelMatch?.[1] ?? labelMatch?.[2] ?? '').toLowerCase();
...
function validateNotificationRules(rules) {
  ...
  for (const rule of rules) {
    ...
    references.set(
      rule.id,
      [...rule.dsl.matchAll(/\brule:([a-z0-9][a-z0-9-]*)/gi)].map((match) =>
        match[1].toLowerCase(),
      ),
    );
  }
