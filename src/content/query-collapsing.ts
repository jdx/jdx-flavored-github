...
interface QueryCollapsingDependencies {
  ...
  loadMetadata: (candidates: Array<QueryTarget & {reference: PullRequestReference}>) => Promise<
    Array<
      QueryTarget & {
        metadata: PullRequestMetadata;
        reference: PullRequestReference;
...
export function getQueryListItemAuthor(row: Element): string | undefined {
  ...
  for (const link of row.querySelectorAll<HTMLAnchorElement>('a[href*="author"]')) {
    const author = new URL(link.href, location.origin).searchParams
      .get('q')
...
export function createQueryListCollapsing({
  ...
  function clearQueryListDecorations() {
    for (const toggle of document.querySelectorAll('.github-inbox-tuner-list-collapse-toggle')) {
      toggle.remove();
    }
    for (const chevron of document.querySelectorAll('.github-inbox-tuner-list-collapse-chevron')) {
      chevron.remove();
    }
    for (const row of document.querySelectorAll('.github-inbox-tuner-query-member--collapsed')) {
      row.classList.remove('github-inbox-tuner-query-member--collapsed');
    }
    for (const row of document.querySelectorAll('.github-inbox-tuner-query-member--expanded')) {
      row.classList.remove('github-inbox-tuner-query-member--expanded');
    }
    for (const row of document.querySelectorAll('.github-inbox-tuner-query-representative')) {
  ...
  function decorateQueryCollapsedGroup(
    ...
    chevron.className =
      'github-inbox-tuner-collapse-chevron github-inbox-tuner-list-collapse-chevron';
    chevron.type = 'button';
  ...
  function decorateQueryGroups(items: QueryItem[], surface: Surface) {
    ...
    if (surface === 'pulls') {
      const stackItems = items.filter((item): item is QueryItem & {metadata: PullRequestMetadata} =>
  ...
  function decorateCachedQueryListCollapses(surface: Surface) {
    ...
    const targets = getTargets(surface).filter(
      (target) => !target.row.closest('[aria-label*="pinned issues" i]'),
    );
    ...
    for (const {row} of items) {
      row
        .querySelector('.commit-build-statuses')
        ?.parentElement?.classList.add('github-inbox-tuner-list-check-status');
    }
    if (surface !== 'pulls') {
