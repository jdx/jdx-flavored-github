...
export function shouldLoadExtraNotificationPage(state: ExtraNotificationPageState): boolean {
  return (
    state.enabled &&
    state.hasNextPage &&
...
export function getUsableNextPageHref(link?: Element | null): string | undefined {
...
function getNotificationGroup(row: Element): Element | undefined {
  return row.closest('.js-notifications-group') ?? row.parentElement?.parentElement ?? undefined;
}
...
export function appendNotificationRows(document_: Document, rows: HTMLElement[]): HTMLElement[] {
  ...
  for (const [group, items] of rowsByGroup) {
    ...
    const existingList = repository ? listsByRepository.get(repository) : undefined;
    const fetchedGroup = items[0].closest('.js-notifications-group') ? group : undefined;
    if (!existingList && fetchedGroup && lastGroup) {
      const adopted = document_.adoptNode(fetchedGroup) as Element;
      for (const row of adopted.querySelectorAll<HTMLElement>('.notifications-list-item')) {
