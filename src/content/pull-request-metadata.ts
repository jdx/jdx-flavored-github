...
export function parsePullRequestMetadata(
	...
	const checkStatusLabel = checkStatusText
		...
		.toLowerCase();
	const checkStatus: CheckStatus | undefined = checkStatusLabel === 'failing'
		? 'failure'
		: checkStatusLabel === 'pending'
			...
			: checkStatusLabel === 'passing'
				? 'success'
				: undefined;
	const statusBatchElements = [...document_.querySelectorAll<HTMLElement>(
	...
	const getStatusBatch = (headSha?: string) => {
		const element = (
			headSha
				? statusBatchElements.find(candidate => (
					candidate.querySelector<HTMLInputElement>('input[name="oid"]')?.value
						=== headSha
				))
				: undefined
		...
		return element && url
			? {
				fields: [...element.querySelectorAll<HTMLInputElement>('input[name]')]
...
export function parseMergeConflict(payload: any): boolean | undefined {
	...
	if (['CLEAN', 'UNSTABLE', 'HAS_HOOKS', 'BEHIND', 'BLOCKED'].includes(
		mergeStateStatus,
	)) {
		return false;
	}
	const conflictCondition = payload?.mergeRequirements?.conditions?.find(
		(condition: any) => /CONFLICT/i.test(condition.type ?? ''),
	);
