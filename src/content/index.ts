...
import type {ExtensionOptions, NotificationRule, Surface, ViewDefinition} from '../shared/types.js';
import {
...
(() => {
  ...
  function getSurfaceOverride(surface: Surface, repository?: string) {
    return (
      (repository ? options.repositoryViewOverrides?.[repository]?.[surface] : undefined) ??
      (getOwner(repository)
  ...
  function getViews(surface: Surface, repository?: string) {
    ...
    const targetRepository = repository ?? getCurrentRepository();
  ...
  function getDefaultViewId(surface: Surface, repository?: string) {
    const targetRepository =
      repository ?? (surface === 'notifications' ? undefined : getCurrentRepository());
    const override = getSurfaceOverride(surface, targetRepository);
  ...
  function matchesNotificationView(row, viewId) {
    ...
    const view =
      candidates.find((candidate) => candidate.id === viewId) ??
      candidates.find(
        (candidate) => candidate.id === getDefaultViewId('notifications', repository),
      ) ??
      candidates[0];
  ...
  function updateNotificationVisibility() {
    ...
    const rows = filterNotificationRowsForFolder(allRows, showsArchivedNotifications());
  ...
  function getConfiguredNotificationLabels(rules = getNotificationRules()) {
    return [
      ...new Set(rules.flatMap((rule) => dsl.getNotificationQualifierValues(rule.dsl, 'label'))),
    ];
  }
  ...
  async function enrichPullRequestLabelFacts(rows, rules?) {
    const explicitLabels = rules ? getConfiguredNotificationLabels(rules) : undefined;
    ...
    await Promise.all(
      [...rowsByRepository].map(async ([repository, items]) => {
        const labels =
          explicitLabels ?? getConfiguredNotificationLabels(getNotificationRules(repository));
        const labelNumbers = new Map(
          await Promise.all(
            labels.map(
              async (label) =>
                [label, await getPullRequestLabelNumbers(repository, label)] as const,
            ),
          ),
        );
  ...
  function setPullRequestFacts(row, reference, status) {
    ...
    row.dataset.githubInboxTunerOwnPullRequest = String(status.ownNumbers.has(reference.number));
  }
  ...
  async function hydratePullRequestChecksCache() {
    ...
    for (const [repository, value] of Object.entries(stored[pullRequestChecksStorageKey] ?? {})) {
      const searchUsable = value?.updatedAt && Date.now() - value.updatedAt < checksUsableFor;
      const exactUsable =
        value?.exactUpdatedAt && Date.now() - value.exactUpdatedAt < checksUsableFor;
  ...
  async function hydratePullRequestMetadataCache() {
    ...
    for (const [key, entry] of Object.entries(stored[pullRequestMetadataStorageKey] ?? {})) {
      if (
        !entry?.updatedAt ||
  ...
  async function persistPullRequestMetadataCache() {
    const stored = Object.fromEntries(
      [...pullRequestMetadataCache]
        .filter(
          ([, entry]) => entry.value?.author && Date.now() - entry.updatedAt < metadataUsableFor,
        )
  ...
  async function updatePullRequestCheckNotifications() {
    ...
    const currentFolderRows = filterNotificationRowsForFolder(rows, showsArchivedNotifications());
    updateViewBar('notifications');
  ...
  function getGlobalNotificationLink() {
    return [...document.querySelectorAll<HTMLAnchorElement>('a[href="/notifications"]')].find(
      (link) =>
  ...
  function insertHeaderSettingsButton(headerActions: HTMLElement) {
    ...
    button.className = [...nativeButtonClasses, 'github-inbox-tuner-settings-button'].join(' ');
  ...
  const headerSettingsController = createHeaderSettingsController<HTMLElement>({
    getActionGroup: () =>
      document.querySelector<HTMLElement>('[data-testid="top-bar-actions"]') ?? undefined,
    hasButton: () => Boolean(document.querySelector('.github-inbox-tuner-settings-button')),
  ...
  function setGlobalNotificationIndicator(hasFocusedNotifications) {
    ...
    const indicatorClasses = [...link.classList].filter((className) =>
      className.includes('notificationIndicator'),
    );
    if (indicatorClasses.length > 0) {
      link.dataset.githubInboxTunerIndicatorClasses = indicatorClasses.join(' ');
    }
    const savedClasses = (link.dataset.githubInboxTunerIndicatorClasses ?? '')
      .split(/\s+/)
      .filter(Boolean);
    ...
    link.dataset.githubInboxTunerFocusedNotifications = String(hasFocusedNotifications);
  }
  ...
  async function loadNotificationPage(url) {
    ...
    const nextHref = document_.querySelector('a[aria-label="Next"]')?.getAttribute('href');
    return {
  ...
  function countVisibleNotifications() {
    ...
    return rows.filter((row) => matchesNotificationView(row, getActiveNotificationViewId(row)))
      .length;
  }
  ...
  function updateExtraNotificationPagesIndicator(loading) {
    ...
    const anchor = lastRow?.closest('.js-notifications-group') ?? lastRow?.parentElement;
    if (!anchor) {
      return;
    }
  ...
  function resetExtraNotificationPages() {
    if (extraNotificationPagesKey === location.href && extraNotificationPagesAnchor?.isConnected) {
      return;
    }
      ...
      const loadedIds = new Set(
        [
          ...document.querySelectorAll<HTMLElement>(
            '.notifications-list-item[data-notification-id]',
          ),
  ...
  async function getPullRequestMetadata(reference) {
    ...
    if (cached?.complete && Date.now() - cached.updatedAt < 5 * 60 * 1000) {
      return cached.value;
    }
    ...
    if (value?.statusBatch?.url) {
      ...
      const statusResponse = await fetch(new URL(value.statusBatch.url, location.origin), {
        body,
        cache: 'no-store',
      ...
      if (statusResponse.ok) {
        ...
        value = {
          ...value,
          checkStatus: parseCommitStatusPartial(statusPayload['item-0'] ?? '') ?? value.checkStatus,
        };
      }
    }
  ...
  function getCachedPullRequestGroupingMetadata(reference) {
    const cached = pullRequestMetadataCache.get(`${reference.repository}#${reference.number}`);
    return cached?.value?.author && Date.now() - cached.updatedAt < metadataUsableFor
      ? cached.value
      : undefined;
  }
  ...
  function placeGroupRowsInOrder(group) {
    ...
    if (firstIndex < 0 || rows.every((row, index) => children[firstIndex + index] === row)) {
      return;
    }
  ...
  function decorateCollapsedGroup(group, signature, label, expandedLabel) {
    ...
    const updateExpandedState = (expanded) => {
      ...
      button.classList.toggle('github-inbox-tuner-collapse-toggle--expanded', expanded);
  ...
  function decorateNotificationGroups(
    items: Array<{
  ...
  async function enrichExactPullRequestMetadata(rows) {
    ...
    for (const item of items) {
      ...
      item.row.dataset.githubInboxTunerMergeConflict = String(item.metadata.mergeConflict === true);
      exactStatusesChanged =
        setExactPullRequestCheckStatus(item.reference, item.metadata.checkStatus) ||
        exactStatusesChanged;
      setPullRequestFacts(
        item.row,
        item.reference,
  ...
  async function updateNotificationStacks() {
    ...
    const loadedItems = await loadPullRequestMetadata(lists.flatMap(({candidates}) => candidates));
  ...
  function updateRepositoryViewActions(rows) {
    ...
    for (const list of lists) {
      ...
      const group =
        list.closest('.js-notifications-group') ??
        list.closest('section, [data-repository-hovercards-enabled], .js-navigation-container') ??
        list.parentElement;
      if (!repository || !group) {
        continue;
      }
      ...
      const button = document.createElement('button');
      button.className =
        `${nativeAction.className} github-inbox-tuner-repository-view-action`.trim();
      button.type = 'button';
  ...
  function getBulkTarget(row, surface) {
    ...
    return {
      ...
      title:
        row
          .querySelector('.markdown-title, [data-testid="issue-row-title-link"]')
  ...
  function createBulkActionsMenu(entries, title = 'Bulk actions') {
    ...
    for (const entry of entries) {
      ...
      button.append(
        document.createTextNode(`${entry.action.label} (${entry.targets.length.toLocaleString()})`),
      );
      button.addEventListener('click', () => {
        details.open = false;
  ...
  async function executeNativeListStep(step, targets) {
    for (const target of targets) {
      const checkbox = target.row.querySelector(
        'input[type="checkbox"]',
    ...
    const trigger =
      [...document.querySelectorAll<HTMLElement>('.js-issue-triage-menu > summary')].find(
        (element) => triggerPattern.test(element.textContent.trim()),
      ) ??
      (await waitForBulkControl(
        (element) =>
    ...
    const option = await waitForBulkControl((element) =>
      optionPattern.test(element.textContent.trim()),
    );
    if (!option) {
    ...
    if (labelAction) {
      ...
      const applyLabels = await waitForBulkControl((element) =>
        /^(Apply|Save changes)$/i.test(element.textContent.trim()),
      );
      applyLabels?.click();
    } else {
      option.click();
  ...
  async function executeBulkAction(entry, onProgress) {
    ...
    for (const [stepIndex, step] of entry.action.steps.entries()) {
      onProgress(
        `Step ${stepIndex + 1} of ${entry.action.steps.length}: ${describeBulkStep(step)}…`,
      );
      if (step.type === 'open') {
  ...
  function getNotificationsGlobalListHost() {
    const notificationList =
      document.querySelector('.js-notifications-list') ??
    ...
    while (globalListHost.parentElement && globalListHost.parentElement !== contentColumn) {
  ...
  function getOwnerSurfaceConfig(surface: Surface, owner?: string) {
    ...
    return override?.views?.length > 0 ? clone(override) : getGlobalSurfaceConfig(surface);
  }
    ...
    const entries =
      surface === 'notifications'
        ? [
            [
              'rule:rule-id',
              'Reuse another named rule. Rule IDs must be unique and cannot form cycles.',
            ],
            [
              'AND · OR · NOT · ( )',
              'Combine conditions explicitly. Whitespace also means AND, and -condition is shorthand for NOT condition.',
            ],
            [
              'repo: · org: · author: · reason:',
              'Match notification metadata. Use author:@me for your own pull requests.',
            ],
            [
              'is: · draft: · conflict: · status: · label: · bot:',
              'Match item type, draft state, merge conflicts, checks, labels, or bot activity. Quote labels containing spaces.',
            ],
            ['title:/pattern/i', 'Match notification titles with a regular expression.'],
            [
              'Default view',
              'Controls the initial view and the global notification indicator. The live new-notification banner follows the selected view chip.',
            ],
            [
              'View chip / Filtered reason',
              'A view chip selects matching rows; a filtered-reason rule creates a pill that can reveal rows it excluded.',
            ],
          ]
        : surface === 'pulls'
          ? [
              [
                'is:open',
                'Limit the view to open items. Add is:pr or is:issue to specify the item type.',
              ],
              ['author:@me · assignee:@me', 'Match items created by or assigned to you.'],
              [
                'label:name · -label:name',
                'Include or exclude labels. Quote labels containing spaces.',
              ],
              ['user-review-requested:@me', 'Match pull requests waiting for your review.'],
              [
                'status:failure · draft:false',
                'Match failing checks or ready-for-review pull requests.',
              ],
  ...
  function openInlineViewEditor(
    ...
    const title = document.createElement('strong');
    const surfaceLabel =
      surface === 'pulls' ? 'pull request' : surface === 'issues' ? 'issue' : 'notification';
    ...
    const getSelected = () => getItems().find((item) => item.id === selectedId) ?? getItems()[0];
    const ensureDefault = () => {
      const visibleItems =
        surface === 'notifications' ? getItems().filter((item) => item.showAsView) : getItems();
    ...
    const inheritedScopeLabel =
      repository && options.ownerViewOverrides?.[getOwner(repository)]?.[surface]
    ...
    save.addEventListener('click', async () => {
      try {
        ...
        if (surface === 'notifications' && !value.rules.some((rule) => rule.showAsView)) {
  ...
  function getQueryPageAnchor() {
    const searchInput =
      document.querySelector('main #js-issues-search') ??
  ...
  function updateViewChipCount(chip: HTMLElement | null, count, activeViewId) {
    ...
    if (countElement) {
      countElement.textContent = Number.isFinite(count) ? count.toLocaleString() : '–';
    }
  ...
  function fitViewBar(bar, force = false) {
    ...
    bar.classList.remove('github-inbox-tuner-views--compact');
  ...
  function updateViewBulkActions(bar, surface, activeViewId) {
    ...
    const menu = createBulkActionsMenu(view.actions.map((action) => ({action, surface, targets})));
  ...
  function updateViewBar(surface: Surface) {
    ...
    if (!bar) {
      ...
      bar.setAttribute(
        'aria-label',
        `${surface === 'pulls' ? 'Pull request' : surface} saved views`,
      );
      if (surface === 'notifications') {
    ...
    const activeViewId =
      (surface === 'notifications' ? activeNotificationView : getActiveQueryView(surface)) ??
      defaultView.id;
    ...
    if (surface === 'notifications') {
      ...
      for (const view of surfaceViews) {
        ...
        updateViewChipCount(bar.querySelector(`[data-view-id="${view.id}"]`), count, activeViewId);
      }
      fitViewBar(bar, true);
    } else {
  ...
  async function fetchQueryCount(surface, query) {
    ...
    const scopedQuery =
      repository && !/(?:^|\s)repo:/i.test(query) ? `${query} repo:${repository}` : query;
  ...
  function redirectToDefaultView(surface: Surface) {
    ...
    const view =
      getViews(surface).find((candidate) => candidate.id === getDefaultViewId(surface)) ??
      getViews(surface)[0];
    if (!view) {
      return false;
    }
  ...
  document.addEventListener(
    ...
    (event) => {
      ...
      const control = event.target.closest(
        'button[aria-label="Done"], .js-grouped-notifications-mark-all-read-button button',
      );
      if (
        !control ||
  ...
  const observer = new MutationObserver((mutations) => {
    ...
    const onlyExtensionControlsChanged =
    ...
    const notificationStateChanged = mutations.some((mutation) => {
      ...
      return ['notification-archived', 'notification-unread'].some(
        (className) => oldClasses.has(className) !== target.classList.contains(className),
      );
    });
  ...
  chrome.storage.onChanged.addListener((changes, areaName) => {
    ...
    if (changes.viewOverrides || changes.ownerViewOverrides || changes.repositoryViewOverrides) {
      activeNotificationView = getDefaultViewId('notifications');
      notificationViewExplicitlySelected = false;
      notificationDslCache.clear();
    }
