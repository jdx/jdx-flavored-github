...
export function getNotificationRepository(element?: Element): string | undefined {
	...
	if (itemLink) {
		const match = new URL(itemLink.href, location.origin)
			.pathname.match(/^\/([^/]+)\/([^/]+)(?:\/|$)/);
		if (match) {
	...
	for (const link of group?.querySelectorAll<HTMLAnchorElement>('a[href]') ?? []) {
		const match = new URL(link.href, location.origin)
			.pathname.match(/^\/([^/]+)\/([^/]+)(?:\/|$)/);
		if (
			match
...
export function hasOnlyVisibleBotParticipants(row: Element): boolean {
	...
	return participants.length > 0
		&& participants.every(link => link.pathname.startsWith('/apps/'));
}
...
function getNotificationMetadata(row: Element) {
	const link = row.querySelector<HTMLElement>(
		'.notification-list-item-link[data-hydro-click]',
	);
	if (!link) {
		return {};
	}
...
export function getNotificationFacts(row: HTMLElement): NotificationFacts {
	...
	const itemLink = row.querySelector<HTMLAnchorElement>(
		'.notification-list-item-link[href]',
	);
	...
	return {
		...
		issue: Boolean(row.querySelector(
		...
		closedIssue: Boolean(row.querySelector(
			':is(.octicon-issue-closed, .octicon-skip)',
		)),
	};
}
...
export function getPullRequestReference(row: Element): PullRequestReference | undefined {
	...
	return match
		? {number: Number(match[3]), repository: `${match[1]}/${match[2]}`}
		: undefined;
}
