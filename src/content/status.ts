...
function updateMergeConflictIcon(row: HTMLElement, facts: NotificationFacts): void {
  ...
  conflictIcon.classList.add('octicon', 'octicon-alert-fill', 'github-inbox-tuner-conflict-icon');
...
export function updateStatusBadges(row: HTMLElement, facts: NotificationFacts): void {
  ...
  if (!checkBadge) {
    ...
    const identifier = title.parentElement?.querySelector<HTMLElement>(
      ':scope > .d-flex > p.m-0.f6.flex-auto > span',
    );
    if (identifier) {
...
export function updateRevealedIndicator(row: HTMLElement, reasons: string[]): void {
  ...
  if (revealed) {
    row.setAttribute('aria-description', `Temporarily revealed; filtered by ${reasons.join(', ')}`);
  } else {
