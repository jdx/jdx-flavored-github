...
(() => {
	...
	function getScopeOwner(scope = state.currentScope) {
		return getScopeKind(scope) === 'owner'
	...
	function getDisplayedSurface(surface) {
		...
		return getRepositorySurface(state.currentScope, surface)
			?? getInheritedSurface(surface);
	}
	...
	function ensureEditableSurface(surface) {
		...
		if (!isSurfaceValue(surface, state.repositories[state.currentScope][surface])) {
			state.repositories[state.currentScope][surface] = clone(
				getInheritedSurface(surface),
			);
		}
	...
	function ensureDefault(surface, value) {
		const candidates = surface === 'notifications'
			? value.rules.filter(rule => rule.showAsView)
			: value.views;
	...
	function renderSurface(surface) {
		...
		const parentLabel = scopeKind === 'repository'
			&& getOwnerSurface(getScopeOwner(), surface)
		...
		const items = getItems(surface, value);
		const selected = items.find(item => item.id === selectedBySurface[surface])
		...
		master.replaceChildren(...items.map((item, index) => {
		...
		if (selected) {
			...
			dslLabel.append(document.createTextNode(
	...
	function renderScopeOptions() {
		...
		repositoryGroup.append(
			...repositories.map(repository => new Option(repository, repository)),
		);
		...
		const validScope = state.currentScope === 'global'
			|| (
				getScopeKind() === 'owner'
	...
	function renderEditors() {
		...
		if (kind === 'owner') {
			...
			scopeHelp.textContent = 'Global views apply unless a user, organization, or repository supplies an override.';
		}
	}
	...
	function normalizeImportedSettings(imported) {
		...
		for (const [repository, overrides] of Object.entries(
			normalized.repositoryViewOverrides,
		)) {
	...
	function buildUpdatedOptions() {
		...
		for (const [repository, repositoryOverrides] of Object.entries(state.repositories)) {
			...
			for (const surface of surfaces) {
				...
				if (
					isSurfaceValue(surface, value)
					&& !isSame(value, inherited)
				) {
		...
		for (const [owner, ownerOverrides] of Object.entries(state.owners)) {
			for (const surface of surfaces) {
				...
				if (
					isSurfaceValue(surface, value)
					&& !isSame(value, state.global[surface])
				) {
